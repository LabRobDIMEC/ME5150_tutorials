...
def get_drone_next_pose(img):
    ...
    print('pos_drone', pose_drone)
    drone_next_pose = [0, 0, 1, 0, 0, 0] # Ejemplo: lista de poses [x, y, z, roll, pitch, yaw]
    ...
    if arucos_data != []:
        ...
        for detection in arucos_data:
            if detection['id'] == 1:
                aruco_position = detection['position']
                new_aruco_position = perm_ejes(aruco_position)
                a = new_aruco_position + [1]
                cam_aruco_position = rot_y(np.deg2rad(20)) @ a 
                print(cam_aruco_position)
                aruco_pos_drone = traslation(z = 0.15) @ cam_aruco_position
                print(aruco_pos_drone)
                pos_aruco = aruco_pos_drone[:3]
                next_aruco = np.array(pos_aruco) + np.array(pose_drone) + np.array([0,0,1])
                print(next_aruco)
                drone_next_pose = list(next_aruco) + [0, 0, 0] 
